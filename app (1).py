...
def create_mashup(selected_songs, hype_level, output_file='mashup.mp3'):
    ...
    for song in selected_songs:
        ...
        for segment_start in best_segments[:num_segments]:
            ...
            # Check if the audio_segment is too short for crossfade
            if len(mashup) > 0 and len(audio_segment) >= 1000:  # Ensure there's something to crossfade
                mashup = mashup.append(audio_segment, crossfade=1000)  # Append with crossfade
            else:
                mashup += audio_segment  # Append without crossfade if too short or if it's the first segment
